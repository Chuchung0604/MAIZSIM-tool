...
class ReadWea:
    ...
    def __init__(self,yr1):
        ...
        if yr1 >= 2025:
            self.isFuture = True
            self.solRad_df = pd.read_csv("G:\TCCIP WEA\臺灣歷史氣候重建資料_5km\日射量平均_1980-2020.csv")
            self.rh_df = pd.read_csv("G:\TCCIP WEA\臺灣歷史氣候重建資料_5km\相對濕度_1980-2020.csv")
            self.windspeed_df = pd.read_csv("G:\TCCIP WEA\臺灣歷史氣候重建資料_5km\平均風速_1980-2020.csv")
        else:
            self.isFuture = False
            #self.solRad_df = []
            
    
    def futureWea(self,lon,lat,site,rcp,model):
        self.Tmax = self.readValueFut(lon,lat,self.year1,"最高溫",site,rcp,model)
        self.Tmin = self.readValueFut(lon,lat,self.year1,"最低溫",site,rcp,model)
        self.Rain = self.readValueFut(lon,lat,self.year1,"降雨量",site, rcp,model)
        self.SolRad = self.solRad_df.loc[(self.solRad_df['LON'] == lon) & (self.solRad_df['LAT'] == lat)].values.flatten().tolist()
        self.SolRad = self.SolRad[2:]
        self.rh = self.rh_df.loc[(self.rh_df['LON'] == lon) & (self.rh_df['LAT'] == lat)].values.flatten().tolist()
        self.rh = self.rh[2:]
        self.windSpeed = self.windspeed_df.loc[(self.windspeed_df['LON'] == lon) & (self.windspeed_df['LAT'] == lat)].values.flatten().tolist()
        self.windSpeed = self.windSpeed[2:]
    
    def histWea(self,lon,lat):
        self.Tmax = self.readValue(lon,lat,self.year1,"最高溫")
        self.Tmin = self.readValue(lon,lat,self.year1,"最低溫")
        self.SolRad = self.readValue(lon,lat,self.year1,"日射量") # (MJ/m2)
    
    def histWeaBatch(self,lon,lat,site):
        self.Tmax = self.readValueBach(lon, lat, self.year1, "最高溫", site)
        self.Tmin = self.readValueBach(lon, lat, self.year1, "最低溫", site)
        self.SolRad = self.readValueBach(lon, lat, self.year1, "日射量", site) #(MJ/m2)
    
    def dayNumbers(self,year):
        # 因為氣象變遷資料都是365天，更改return value
    ...
    def readValueBach(self,lon,lat,year,weatype,loc):
        # read the daily temperature in specsific grid, combine two years
        # to make a long list 
...
#     Rad = Radiation(24.3) # activate class with latitude
...
#         tempH.append(test[h])
# =============================================================================
    solRad_df = pd.read_csv("G:\TCCIP WEA\臺灣歷史氣候重建資料_5km\日射量平均_1980-2020.csv") 
    #solRad_df.drop(solRad_df.columns[len(solRad_df.columns)-1], axis=1, inplace=True)
    test = solRad_df.loc[(solRad_df['LON'] == 120.75) & (solRad_df['LAT'] == 24.3)].values.flatten().tolist()
    #test.extend(test[2:len(test)])
    test = test[2:]
    #test =  test[2:]
